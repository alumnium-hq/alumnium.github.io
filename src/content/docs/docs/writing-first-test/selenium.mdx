...
Let's see how we can write a simple test that would use [Selenium][5] to open DuckDuckGo, search for a Mercury element, ensure that search results are loaded, include the Wikipedia article about the element, and check that its symbol is presented correctly on the page.
...
We'll also need to install Docker to run Alumnium server. Please, refer to [Docker installation guide][4] for your platform.
